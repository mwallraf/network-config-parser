...
class CpeModule(object):
    ...
    allow_ip_as_hostname = (
        False  # in dryrun method, allows filenames which exist of ip address
    )
    ...
    pe_interface_ip = (
        []
    )  # list of all PE IP addresses, to quickly check if an IP was already processed
    ...
    def _store_router_object(self, rtr):
        ...
        if rtr.isCPE():
            log.debug(
                "Storing P2P interfaces for CPE router object: {}".format(
                    rtr.GetProp("hostname")
                )
            )
            l = self.p2p_objects
            ...
            if not len(rtr.getP2PInterfaces()) > 0:
                log.error(
                    "Router {} does not have any P2P interfaces!".format(
                        rtr.GetProp("hostname")
                    )
                )
            ...
            log.debug(
                "Storing P2P interfaces for PE router object: {}".format(
                    rtr.GetProp("hostname")
                )
            )
            l = self.pe_objects
    ...
    def _dryrun_method(self):
        ...
        if not self.dryrun_config_folder:
            log.warning(
                "config folder {} not found - exit script".format(
        ...
        configs = [
            f
            for f in os.listdir(self.dryrun_config_folder)
        ...
        for c in configs:
            ...
            if not self.allow_ip_as_hostname:
                if re.match("[0-9]+\.[0-9]+\.[0-9]+\.[0-9]", routername):
                    log.info(
                        "skipping hostname '{}' because ip addresses are not allowed".format(
                            routername
                        )
                    )
                    continue
            ...
            if config_function not in supported_routertypes:
                log.warn(
                    "unsupported router type '{}', skip config file: {}".format(
                        config_function, c
                    )
                )
                continue
            ...
            log.debug(
                "Opening config file: {}".format(
                    "{}/{}".format(self.dryrun_config_folder, c)
                )
            )
            with open(
                "{}/{}".format(self.dryrun_config_folder, c), encoding="latin-1"
            ) as configfile:
            ...
            if int(self.dryrun_config_age) > 0 and config_function == "CPE":
                log.debug(
                    "check if config is less then {} days old".format(
                ...
                if int(rtr.getLastSeen()) < int(oldest_time):
                    log.warning(
                        "config is older than the expected {} days ({}) -- skipping: {}".format(
                            self.dryrun_config_age, oldest_time, c
                        )
                    )
                    continue
                else:
    ...
    def ParseAllDevices(self):
        ...
        for type in types:
            ...
            if type == "PE":
                ...
                self.pe_interface_ip = [
                    o.ip for x in self.pe_objects for o in self.pe_objects[x]
                ]
    ...
    def _is_cpe(self, h):
        ...
        if ip in self.pe_interface_ip:
            log.warning(
                "IP address {} is known as an PE interface, do not process as CPE".format(
                    ip
                )
            )
            return False
