...
class Router(object):
    ...
    def _parse_config_header(self, keywords):
        for kw in keywords:
            ...
            if len(lines) > 0:
                log.debug("property found in header: %s = %s" % (kw, lines[0].split(": ")[-1]))
                ...
                log.warn("property %s is not found in the config header" % kw)
    ...
    def _parse_interfaces(self):
        ...
        for l in lines:
            ...
            if not (m and len(m.groups())) > 0:
                ...
                log.debug("---> new L3 interface found: %s - %s" % (_intf, _ip))
                try:
                    ...
                    while DOCONTINUE:
                        ...
                        if m:
                            ...
                            if len(intf_cfg) > 0:
                                ...
                                log.warn("No substitute found for %s so assuming it's generated by a dialer interface" % _intf)
                                DOCONTINUE = False
                                continue
                        DOCONTINUE = False
