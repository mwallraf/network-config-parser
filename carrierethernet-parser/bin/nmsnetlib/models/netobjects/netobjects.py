...
class _base(object):
    ...
    def json(self, allownull=False):
        ...
        j = { k: _object_to_string(self.__dict__[k]) for k in self.__dict__ if (allownull is True or (type(self.__dict__[k]) is not list and self.__dict__[k] is not None) or (type(self.__dict__[k]) is list and len(self.__dict__[k]) > 0)) }
...
class _ring(_base):
    def __init__(self, **kwargs):
        ...
        self.ringstatus = kwargs.get('ringstatus', None)  # ring status: open or closed
...
class _logicalring(_ring):
    def __init__(self, **kwargs):
        ...
        self.ringmembers = self._init_list(kwargs.get('ringmembers'))  # list of all neighbors in the ring
        self.virtualrings = self._init_list(kwargs.get('virtualrings'))  # link to all virtual rings connected to this ring
...
class _virtualring(_ring):
    def __init__(self, **kwargs):
        ...
        self.logicalring = kwargs.get('logicalring', None)  # name of logical ring this ring belongs to
        self.rapsvid = kwargs.get('rapsvid', None)
        self.subring = kwargs.get('subring', None)  ## this contains east-port-termination, west-port-termination, no-port-termination
        self._eastport_termination = self._init_list(kwargs.get('_eastport_termination'))  ## generated - will contain all hostnames in the vring that have subring east-port-termination configured
        self._westport_termination = self._init_list(kwargs.get('_westport_termination'))  ## generated - will contain all hostnames in the vring that have subring west-port-termination configured
        #self.eastportrpl = kwargs.get('eastportrpl', None)
        self.rplownerport = self._init_list(kwargs.get('rplownerport'))  
        self.rplowner = self._init_list(kwargs.get('rplowner'))  # hostname that has an rplownerport
        self.vlans = self._init_list(kwargs.get('vlans')) ## link to the S-vlans assigned to this VR
        self.vswitches = self._init_list(kwargs.get('vswitches'))  ## link to the vswitches assigned to this VR
...
class _port(_base):
    def __init__(self, **kwargs):
        ...
        self._vlans = self._init_list(kwargs.get('_vlans')) ## links to vlans assigned to this port
        self.description = kwargs.get('description', None)  ## ex 0/1, 1
        self.operstate = kwargs.get('operstate', None)  ## enabled|disabled|Up|Down
        self.adminstate = kwargs.get('adminstate', None)  ## enabled|disabled|Up|Down
        self.qosmapin = kwargs.get('qosmapin', None)  ## name of inbound QOS map
        self.qosmapout = kwargs.get('qosmapout', None)  ## name of inbound QOS map
        ...
        self.defaultvlanid = kwargs.get('defaultvlanid', None)  ## default vlan id this port has
        self.linkstateduration = kwargs.get('linkstateduration', None)  ## 3d13h21m
        self.speedcapability = kwargs.get('speedcapability', None)  ## 10/100/G
        self.adminspeedduplex = kwargs.get('adminspeedduplex', None)  ## 100FD, 10HD, ..
        self.operspeedduplex = kwargs.get('operspeedduplex', None)  ## 100FD, 10HD, ..
...
class _switchport(_port):
    def __init__(self, **kwargs):
        ...
        #self._id = 'portidx'
        self.type = kwargs.get('type', 'L2 switchport')  ## MSTP enabled or disabled
        ...
        self.vplsporttype = kwargs.get('vplsporttype', None)  ## Subsc = UNI or Ntwrk = NNI
        self.ethvcethertype = kwargs.get('ethvcethertype', None)  ## usually 8100 on SDS
        self.egressshaper = kwargs.get('egressshaper', None)  ## ex. shaper-rate 10000
        self.egressqueue = kwargs.get('egressqueue', None)  ## ex. weighted-deficit-round-robin
...
class _lagport(_port):
    def __init__(self, **kwargs):
        ...
        self.type = kwargs.get('type', 'L2 lag port')  ## MSTP enabled or disabled
...
class _vlan(_base):
    def __init__(self, **kwargs):
        ...
        self.ports = self._init_list(kwargs.get('ports')) # link to ports that have this vlan assigned
        self.type = self._init_list(kwargs.get('type'))  # ex. reserved-vlan, virtual-circuit, SVLAN, CVLAN
        #self.ports = kwargs.get('ports', [])
        #self.type = kwargs.get('type', [])  # ex. reserved-vlan, virtual-circuit, SVLAN, CVLAN
        self._vswitches = self._init_list(kwargs.get('_vswitches')) ## links to the vswitches that have this vlan
        self._virtualrings = self._init_list(kwargs.get('_virtualrings')) ## links to the vswitches that have this vlan
...
class _service(_base):
    def __init__(self, **kwargs):
        ...
        self._id = 'serviceid'
        self.serviceid = kwargs.get('serviceid', None) # this could be a L2 service id (VT or something else) or a customer facing service id (= VT), normally the vswitch name
        self.description = kwargs.get('description', None)
...
class _tagged_vlan(_vlan):
    def __init__(self, **kwargs):
        ...
        self.type = self._init_list(kwargs.get('type')) + [ 'TAGGED_VLAN' ]  # ex. reserved-vlan, virtual-circuit, SVLAN, CVLAN
...
class _vcircuit(_base):
    def __init__(self, **kwargs):
        ...
        self._vswitches = kwargs.get('_vswitches', []) ## link to all the virtual switches assigned
...
class _vswitch(_base):
    def __init__(self, **kwargs):
        ...
        self.virtualinterfaces = kwargs.get('virtualinterfaces', [])  ## this is for SAOS 87xx
        self.serviceid = kwargs.get('serviceid', None)  # service id's found in the vswitch description (VT or GSID)
...
class _tacacs(_base):
    def __init__(self, **kwargs):
        ...
        self.authorization = kwargs.get('authorization', None)  # authorization is enabled or disabled
        self.accounting = kwargs.get('accounting', None)  # accounting is enabled or disabled
        self.accounting_session = kwargs.get('accounting_session', None)  # accounting is enabled or disabled
        self.accounting_command = kwargs.get('accounting_command', None)  # accounting is enabled or disabled
