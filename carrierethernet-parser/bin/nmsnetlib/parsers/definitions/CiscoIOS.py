...
definition = {
            'hostname': {
                    'regex': [ 
                        { 'mapper': 'HOST', 're': re.compile("(?P<hostname>.*)") }
                    ],
            },
            'interface': {
                    'regex': [
                    ...
                    'preprocessor': {
                        'regex': [
                            re.compile("^(?P<CREATEBLOCKNAME>interface .*)")
                        ]
                    }
            },
            ...
            'dynamic_blocks': [
                re.compile("^(?P<DYNAMICBLOCKNAME>hostname) (?P<CONFIG>.*)$"),
                #re.compile("^(?P<DYNAMICBLOCKNAME>interface) (?P<CONFIG>.*)$")
            ],
        }
