...
class ParserCollection(object):
    ...
    def _linkcollection_lldp(self):
        ...
        for s in self.parserlist:
            ...
            for n in s.model.lldpneighbors:
                ...
                #print("b = {}".format(b))
                nbr = next(iter(list(filter(lambda x: str(x.model.host.chassisid).upper() == str(n.nbr), self.parserlist))), None)
                #print("nbr = {}".format(nbr))
                if nbr:
                    #print("nbr has been set for {} - {} - {}".format(s.model.host.hostname, n.nbr, nbr))
                    n.nbr = nbr
                    nbrport = next(iter(list(filter(lambda x: str(x) == str(n.nbrPort), nbr.model.switchports))), None)
                    n.nbrPort = nbrport
    ...
    def get_ring_status_code(self, ring):
        ...
        """
        categories = { 'OK': range(0, 1), 'OK - WARNINGS': range(1, 10), 'NOT OK': range(10, 20) }
        mapping = { }
        ...
        for category in errors:
            if errors[category].get('count', 0) > 0:
                ...
                highest_weight = new_weight if new_weight > highest_weight else highest_weight

        status_code = "OK" if highest_weight is None else mapping[str(highest_weight)]
    ...
    def get_severity_from_errors(self, errors):
        """
            ...
            Severity:
                weight: INFO, WARN, ERROR, CRITICAL
                         0-4, 5-9, 10-14, 15-19
        """
        categories = { 'INFO': range(0, 5), 'WARN': range(5, 10), 'ERROR': range(10, 15), 'CRITICAL': range(15, 20) }
        mapping = { }
        ...
        severity = "OK" if highest_weight is None else mapping[str(highest_weight)]
        #print("Severity: {}".format(severity))
    ...
    def get_netjson_lldp(self):
        ...
        for p in self.parserlist:
            pjson = json.loads(p.model.get_netjson_lldp() or '{ "nodes": [], "links": [] }')
            #print("pjson = {}".format(pjson))
            for node in pjson['nodes']:
                #print("node = {}".format(node))
                n = next(iter(list(filter(lambda x: str(x['id']) == str(node['id']), nodes))), None)
                if not n:
                    nodes.append(node)
            for link in pjson['links']:
                n = next(iter(list(filter(lambda x: "{}-{}".format(x['source'], x['target']) == "{}-{}".format(link['source'], link['target']), links))), None)
                if not n:
                    links.append(link)
    ...
    def get_switchport_status(self, refresh=False):
        ...
        def create_error_categories(): 
            return { 'vlan_issues': { 'count': 0, 'weight': 5 },        
        ...
        min_rqd_fields = [ "name", "adminstate", "adminspeedduplex", "operstate", "operspeedduplex", "autoneg", "linkstateduration",
                           "description", "lldp", "rstp", "stormcontrol", "xsvrstatus", "lldpnbr", "mtu", "type", "ports", 
                           "_vlans", "speedcapability", "vplsporttype", "egressqueue", "egressshaper"
                         ]
        ...
        def create_record(hostname, port):
            ...
            p["_vlans"] = [ str(v.vlan) if self._is_class(v) else str(v) for v in p["_vlans"] ]

            ## check for errors
            # VLAN 127 should not be present except for SAS87
            if 'SAS87' not in hostname and '127' in p['_vlans']:
                p['errors'].append("{} port {} - Vlan 127 should not be configured on this port".format(hostname, p["name"]))
            ...
            if p['adminstate'] == 'enabled' and '1' in p['_vlans']:
                p['errors'].append("{} port {} (admin state = {}) - Vlan 1 should not be configured on this port".format(hostname, p["name"], p["adminstate"]))
                p['errors_cat']['vlan_issues']['count'] += 1
            # MTU size 9100 for NNI and 9216 for UNI
            if p['adminstate'] == 'enabled' and p['vplsporttype'] == 'NNI' and p['mtu'] != '9100':
                p['errors'].append("{} NNI port {} - MTU size is {} but should be 9100".format(hostname, p["name"], p["mtu"]))
                p['errors_cat']['mtu_issues']['count'] += 1
            if p['adminstate'] == 'enabled' and p['vplsporttype'] == 'UNI' and p['mtu'] != '9216':
                p['errors'].append("{} UNI port {} - MTU size is {} but should be 9216".format(hostname, p["name"], p["mtu"]))
                p['errors_cat']['mtu_issues']['count'] += 1
            # duplex issues
            if p['operspeedduplex'] and p['adminstate'] == 'enabled' and 'HD' in p['operspeedduplex']:
                p['errors'].append("{} port {} - duplex issues {}".format(hostname, p["name"], p["operspeedduplex"]))
                p['errors_cat']['duplex_issues']['count'] += 1
            # speed issues
            if p['operspeedduplex'] and p['adminstate'] == 'enabled' and p['egressshaper']:
                m = re.match("(?P<OPERSPEED>[0-9]+).*", p['operspeedduplex'])
                if m:
                    if m.groupdict().get('OPERSPEED', 0) != p['egressshaper']:
                        p['errors'].append("{} port {} - egress shaper mismatch {} with operational speed {}".format(hostname, p["name"], p["egressshaper"], p['operspeedduplex']))
        ...
        for p in self.parserlist:
            ...
            hostname = p.model.host.hostname or p.model.host.chassisid or UNKNOWN
        ...
        for node in self.switchportinfo['nodes']:
            json_output.append(json.dumps(node, indent=4, separators=(',', ': ')))
        return "\n".join(json_output)
    ...
    def _logical_ring_status(self, logicalring):
        ...
        lldp_members = []
        ring_members = sorted(list(set([ x.model.host.hostname if self._is_class(x) else x for x in logicalring['ringmembers'] ])), key=lambda x: x)

        for rmember in logicalring['ringmembers']:
            self.logger.debug("ringmember: {}".format(rmember))

            if not self._is_class(rmember):
                logicalring['errors'].append("ring member {} is not an object".format(rmember))
                logicalring['errors_cat']['lldp_issues']['count'] += 1
                continue

            mhostname = rmember.model.host.hostname
            # find the logical ring for this member
            lr = next(iter(list(filter(lambda x: (x.name == logicalring['name']) and (x.type == 'logical-ring'), rmember.model.rings ))), None)
            self.logger.debug("logical ring found for the ring member: {}".format(lr))

            if not (lr.eastport and lr.westport):
                logicalring['errors'].append("{} - {}: missing logicalring port".format(mhostname, lr.name))
            ...
            for port in [ lr.eastport, lr.westport ]:
                ...
                if not self._is_class(port):
                    logicalring['errors'].append("{} - {} - {}: logicalring ports are unknown".format(mhostname, lr.name, port.name))
                ...
                if not self._is_class(lldpnbr):
                    logicalring['errors'].append("{} - {} - {} - {}: lldp neighbor is not an object".format(mhostname, lr.name, port.name, lldpnbr))
                    logicalring['errors_cat']['lldp_issues']['count'] += 1
                    logicalring['errors_cat']['ring_open_issues']['count'] += 1
                    self.logger.debug("LLDP ISSUE - lldp neighbor is not an object - {}".format(lldpnbr))
                    continue
                ...
                if lldpnbr:
                    ...
                    logicalring['errors'].append("{} - {} - {}: no LLDP neighbor found for port".format(mhostname, lr.name, port.name))
                    logicalring['errors_cat']['lldp_issues']['count'] += 1
                    logicalring['errors_cat']['ring_open_issues']['count'] += 1
                    continue
        ...
        if not set(lldp_members) == set(ring_members):
            logicalring['errors'].append("LLDP neighborships [{}] do not match ring members [{}]".format(", ".join(lldp_members), ", ".join(ring_members)))
            logicalring['errors_cat']['lldp_issues']['count'] += 1
            logicalring['errors_cat']['ring_open_issues']['count'] += 1
    ...
    def get_logical_ring_info(self, refresh=False):
        ...
        #         0-4, 5-9, 10-14, 15-19
        def create_error_categories(): 
            return { 'lldp_issues': { 'count': 0, 'weight': 5 },        
        ...
        for p in self.parserlist:
            ...
            for lring in p.model.logical_rings():
                ...
                vrings = [ x.name if self._is_class(x) else x for x in lring.virtualrings ] or []
                ## check if the ring already exists, if it does then add the ringmember, otherwise add the ring
                n = next(iter(list(filter(lambda x: "{}-{}".format(x['name'], x['id']) == rkey, logicalrings))), None)
                if n:
                    n['ringmembers'].append(p)
                    n['virtualrings'] = n['virtualrings'] + vrings
                else:
                    newring = { 'name': rname, 'status': UNKNOWN, 'ringnamekey': [], 'id': rid, 'virtualrings': vrings, 'ringmembers': [ p ], 'errors': [], 'errors_cat': create_error_categories() }
        ...
        for lring in logicalrings:
            ...
            lring['severity'] = self.get_severity_from_errors(lring["errors_cat"])
            lring['virtualrings'] = list(set(lring['virtualrings']))
            lring['ringmembers'] = list(set([ x.model.host.hostname if self._is_class(x) else x for x in lring['ringmembers'] ]))
        ...
        for lr in self.logicalringinfo['logicalrings']:
            json_output.append(json.dumps(lr, indent=4, separators=(',', ': ')))
        return "\n".join(json_output)
    ...
    def _virtual_ring_status(self, virtualring):
        ...
        for rmember in virtualring['ringmembers']:
            if not self._is_class(rmember):
                virtualring['errors'].append("{} - ring member {} is not an object".format(virtualring['name'], rmember))
                virtualring['errors_cat']['lldp_issues']['count'] += 1
                continue
            mhostname = rmember.model.host.hostname
            # find the virtual ring for this member
            vr = next(iter(list(filter(lambda x: (x.name == virtualring['name']) and (x.type == 'virtual-ring'), rmember.model.rings ))), None)
            ...
            if not vr.logicalring:
                virtualring['errors'].append("{} - missing a logical ring, check {}".format(virtualring['name'], mhostname))
                virtualring['errors_cat']['logical_ring_missing']['count'] += 1
            else:
                lr_ports = [ vr.logicalring.eastport, vr.logicalring.westport ]
            if len(lr_ports) <2:
                virtualring['errors'].append("{} - the logical ring is missing east or west port on {}".format(virtualring['name'], mhostname))
                virtualring['errors_cat']['logical_ring_missing']['count'] += 1
            ...
            virtualring_vlans = [ x.vlan if self._is_class(x) else x for x in virtualring['vlans'] ]
            if set(vr_vlans) != set(virtualring_vlans):
                ...
                virtualring['errors'].append("{} - the vlans do not match, check {}. Vlan diff = {}".format(virtualring['name'], mhostname, set(virtualring_vlans).symmetric_difference(set(vr_vlans))))
                virtualring['errors_cat']['vlan_issues']['count'] += 1
            for vlan in vr.vlans:
                vlan_ports = vlan.ports if self._is_class(vlan) else None
                if not vlan_ports:
                    virtualring['errors'].append("{} - vlan {} is not configured on any ports on {}".format(virtualring['name'], str(vlan), mhostname))
                    virtualring['errors_cat']['vlan_issues']['count'] += 1
                # the vlan should be configured on at least the eastport + westport of the logical ring
                else:
                    vlan_ports = [ x.name if self._is_class(x) else x for x in vlan_ports ]
                    lr_ports = [ x.name if self._is_class(x) else x for x in lr_ports ]
                    ...
                    if len(port_diff) > 0:
                        ...
                        virtualring['errors'].append("{} - vlan {} is not configured on all the logical ring ports on {} (lr ports: {}, missing ports: {})".format(virtualring['name'], str(vlan), mhostname, lr_ports, port_diff))
                        virtualring['errors_cat']['vlan_issues']['count'] += 1

        ## don't give an error if the termination ports are set tot -none-
        if not (virtualring['easttermination'] == virtualring['westtermination'] == NONE):
            # exceptions: do not throw errors for core rings
            if re.match("^[LV]R\-CMR[\-_]", virtualring['name']):
                # do not throw an error
                pass
            else:
                if len(virtualring['easttermination']) == 0 or len(virtualring['westtermination']) == 0:
                    virtualring['errors'].append("{} - port termination is missing".format(virtualring['name']))
                    virtualring['errors_cat']['port_termination']['count'] += 1
                if len(virtualring['easttermination']) > 1 or len(virtualring['westtermination']) > 1:
                    virtualring['errors'].append("{} - port termination is defined multiple times, check east-port-termination or west-port-termination".format(virtualring['name']))
                    virtualring['errors_cat']['port_termination']['count'] += 1
        if len(virtualring['ringmembers']) < 2:
            virtualring['errors'].append("{} - not enough nodes in the ring".format(virtualring['name']))
            virtualring['errors_cat']['ring_members_issue']['count'] += 1
        if len(virtualring['vlans']) == 0:
            virtualring['errors'].append("{} - no vlans configured".format(virtualring['name']))
            ...
            if len(unique_vlans[v]) > 0:
                ...
                for v1 in unique_vlans[v]:
                    ...
                    if (descr is None) and name:
                        ...
                        virtualring['errors'].append("{} - vlan {}: naming or description inconsistency".format(virtualring['name'], v1.vlan))
                        virtualring['errors_cat']['vlan_naming_issues']['count'] += 1

        virtualring_members = [ x.model.host.hostname if self._is_class(x) else x for x in virtualring['ringmembers'] ]
        if set(virtualring_members) != set(virtualring['logicalringmembers']):
            virtualring['errors'].append("{} - the ring members do not match the logical ring members".format(virtualring['name']))
            virtualring['errors_cat']['ring_members_issue']['count'] += 1

        # check rplowner (exactly 1, not the same as east or west termination)
        # if the ringmembers only contain the termination hosts then rplowner = no
        if (len(virtualring['easttermination']) > 0 and len(virtualring['westtermination']) >0) and (set(virtualring_members) == set(virtualring['easttermination'] + virtualring['westtermination'] ) ):
            virtualring['rplowner'] = [ "no" ]
        else:
            if len(virtualring['rplowner']) != 1:
                virtualring['errors'].append("{} - rpl owner count is not correct ({})".format(virtualring['name'], len(virtualring['rplowner'])))
                virtualring['errors_cat']['rplowner_issues']['count'] += 1
            for rplowner in virtualring['rplowner']:
                if (rplowner in virtualring['easttermination']) or (rplowner in virtualring['westtermination']):
                    virtualring['errors'].append("{} - rpl owner should not be the same as east or west termination".format(virtualring['name'], rplowner))
                    virtualring['errors_cat']['rplowner_issues']['count'] += 1

        ### check unique ring name, 1 VR name per RAPSVID-easttermination-westtermination
        vrkey = virtualring['ringnamekey']
        if not vrkey:
            virtualring['errors'].append("{} - unable to check if the ringnamekey is unique, vrapsid or port termination is missing".format(virtualring['name']))
            virtualring['errors_cat']['ring_naming_issues']['count'] += 1
        else:
            if vrkey in self.vringnames:
                if not (len(set(self.vringnames[vrkey])) == 1):
                    virtualring['errors'].append("{} - there is a problem with the virtual ring name, multiple vrings have the same ringnamekey ({}): {}".format(virtualring['name'], vrkey, set(self.vringnames[vrkey])))
                    virtualring['errors_cat']['ring_naming_issues']['count'] += 1
    ...
    def get_virtual_ring_info(self, refresh=False):
        ...
        #         0-4, 5-9, 10-14, 15-19
        def create_error_categories():
            return { 'ring_members_issue': { 'count': 0, 'weight': 10 },
        ...
        for p in self.parserlist:
            ...
            for vring in p.model.virtual_rings():
                ...
                vlans = vring.vlans
                logicalring = vring.logicalring.name if self._is_class(vring.logicalring) else vring.logicalring or UNKNOWN
                ## find the logical ring from the logicalringinfo collection
                lr = next(iter(list(filter(lambda x: x['name'] == logicalring, self.logicalringinfo['logicalrings']))), None)
                logicalringstatus = lr['status'] if lr else UNKNOWN
                logicalringmembers = lr['ringmembers'] if lr else []

                ## check if the ring already exists, if it does then add the ringmember, otherwise add the ring
                n = next(iter(list(filter(lambda x: "{}-{}".format(x['name'], x['rapsvid']) == "{}-{}".format(name, rapsvid), virtualrings))), None)
                ...
                if n:
                    ...
                    n['easttermination'] = n['easttermination'] + eastport_termination
                    n['westtermination'] = n['westtermination'] + westport_termination
                    ...
                    if n['rapsvid'] and (len(et) > 0) and (len(wt) > 0):
                        vringkey = "{}-{}-{}".format(n['rapsvid'], self.coldelim.join(et), self.coldelim.join(wt))
                        n['ringnamekey'] = vringkey
                        self.vringnames.setdefault(vringkey, [])
                        self.vringnames[vringkey].append(n['name'])
                else:
                    newring = { 'name': name, 'ringnamekey': '', 'status': UNKNOWN, 'severity': UNKNOWN, 'rapsvid': rapsvid, 
                                'rplowner': rplowner,'vlans': vlans, 'ringmembers': [ p ], 'easttermination': eastport_termination, 
                                'westtermination': westport_termination, 'logicalring': logicalring, 'logicalringstatus': logicalringstatus, 
                                'logicalringmembers': logicalringmembers, 'errors': [], 'errors_cat': create_error_categories() }
        ...
        for vring in virtualrings:
            ...
            vring['vlans'] = list(set([ "{}".format(x.vlan) if self._is_class(x) else x for x in vring['vlans'] ]))
            #vring['vlans'] = list(set([ "{} ({})".format(x.vlan, x.name or UNKNOWN) if self._is_class(x) else x for x in vring['vlans'] ]))
            vring['ringmembers'] = list(set([ x.model.host.hostname if self._is_class(x) else x for x in vring['ringmembers'] ]))
            vring['easttermination'] = self.coldelim.join( list(set(vring['easttermination'])) )
            vring['westtermination'] = self.coldelim.join( list(set(vring['westtermination'])) )
            vring['severity'] = self.get_severity_from_errors(vring["errors_cat"])
        ...
        for vr in self.virtualringinfo['virtualrings']:
            json_output.append(json.dumps(vr, indent=4, separators=(',', ': ')))
        return "\n".join(json_output)
    ...
    def report_virtual_ring_inventory(self):
        ...
        """
        yield [ "RINGNAME", "STATUS", "SEVERITY", "TYPE", "HOSTNAME", "MGMTIP", "RAPSVID", "RPLOWNER", "VLANCOUNT", "EASTTERMINATION", "WESTTERMINATION", "RINGMEMBERCOUNT", "RINGMEMBERS", "LOGICALRING", "LRSTATUS", "LRMEMBERCOUNT", "LRMEMBERS", "VLANS", "ERRORS", "ERR_RING_NAMING","ERR_RINGMEMBERS", "ERR_NO_VLANS", "ERR_PRT_TERM", "ERR_VLAN", "ERR_VLAN_NAMING", "ERR_LR", "ERR_LLDP", "ERR_RPLOWNER" ]
        ...
        for vr in self.virtualringinfo['virtualrings']:
            for host in vr.get('ringmembers'):
                ...
                _hostname = host
                _mgmtip = "" if not hostobj else hostobj.model.get_management_ip()
                ...
                _lrmembers = str(self.coldelim.join(vr.get('logicalringmembers')))
                _errors = str(len(vr.get('errors')))
                _errors_ring_members = str(vr["errors_cat"]["ring_members_issue"]["count"])
                _errors_no_vlans = str(vr["errors_cat"]["no_vlans_configured"]["count"])
                _errors_port_termination = str(vr["errors_cat"]["port_termination"]["count"])
                _errors_vlan_issues = str(vr["errors_cat"]["vlan_issues"]["count"])
                _errors_vlan_naming_issues = str(vr["errors_cat"]["vlan_naming_issues"]["count"])
                _errors_lr_missing = str(vr["errors_cat"]["logical_ring_missing"]["count"])
                _errors_lldp = str(vr["errors_cat"]["lldp_issues"]["count"])
                _errors_rplowner = str(vr["errors_cat"]["rplowner_issues"]["count"])
                _errors_vr_naming_issues = str(vr["errors_cat"]["ring_naming_issues"]["count"])
                

                yield [ _ringname, _status, _severity, _type, _hostname, _mgmtip, _rapsvid, _rplowner, _vlancount, _easttermination, _westtermination, _ringmembercount, _ringmembers, _logicalring, _lrstatus, _lrmembercount, _lrmembers, _vlans, _errors, _errors_vr_naming_issues, _errors_ring_members, _errors_no_vlans, _errors_port_termination, _errors_vlan_issues, _errors_vlan_naming_issues, _errors_lr_missing, _errors_lldp, _errors_rplowner ]
    ...
    def report_logical_ring_inventory(self):
        ...
        """
        yield [ "HOSTNAME", "MGMTIP", "RINGNAME", "RINGID", "SITEID", "DSRID", "ASRID", "LOCATION", "STATUS", "SEVERITY", "TYPE", "RINGMEMBERCOUNT", "RINGMEMBERS", "VIRTUALRINGCOUNT", "VIRTUALRINGS", "ERRORS", "ERR_RING_NAMING", "ERR_LLDP", "ERR_PORTS", "ERR_LR" ]
        ...
        for lr in self.logicalringinfo['logicalrings']:
            for host in lr.get('ringmembers'):
                ...
                _errors_port_issues = str(lr["errors_cat"]["port_issues"]["count"])
                _errors_lr_issues = str(lr["errors_cat"]["logical_ring_missing"]["count"])
                _errors_lr_naming_issues = str(lr["errors_cat"]["ring_naming_issues"]["count"])

                yield [_hostname, _mgmtip, _ringname, _ringid, _siteid, _dsrid, _asrid, _location, _status, _severity, _type, _ringmembercount, _ringmembers, _virtualringcount, _virtualrings, _errors, _errors_lr_naming_issues, _errors_lldp, _errors_port_issues, _errors_lr_issues ]
    ...
    def _validate_logical_ring_name(self, ringname):
        ...
        """
        rex = re.compile("^LR-(?P<chassistype>[AD]SR)-(?P<siteid>ANT|BRU|DWDM)_(?P<dsrid>[0-9]+)_(?P<asrid>[0-9]+)")
        r = { "chassistype": "", "siteid": "", "dsrid": "", "asrid": "", "compliant": False }
    ...
    def get_service_info(self, refresh=False):
        ...
        for p in self.parserlist:
            ...
            for svc in p.model.services:
                ...
                n = next(iter(list(filter(lambda x: x["serviceid"] == serviceid, services))), None)
                if n:
                    ...
                    new_service = { 'serviceid': serviceid, 'devices': [ pname ], 'descriptions': [], 'cfsid': []  }
                    if description:
        ...
        self.serviceinfo = { 'services': sorted(services, key=lambda k: k['serviceid'], reverse=False) }
    ...
    def report_service_inventory(self):
        ...
        for v in self.serviceinfo['services']:
            ...
            r["DESCRIPTIONS"] = str(self.coldelim.join(set(v.get('descriptions'))))
    ...
    def get_vlan_info(self, refresh=False):
        ...
        def _create_update(v):
            ...
            new_vlan = { '_id': '','vlan': '', 'name': '', 'type': '', 'vr_name': '', 'vr_status': '', 'lr_name': '', 'lr_status': '', 'ports': '', 'vswitch': '', 'terminating_port': '', 'terminating_host': '' }
            for k in v.keys():
                new_vlan[k] = v[k]
                ...
                if 'SVLAN' in vlan_type:
                    if len(v._virtualrings) > 0:
                        for vr in v._virtualrings:
                            ...
                            new_vlan_name = "S-{}-{}".format(vlan_id, vr_rapsvid)
                            ports = ""                           
                            _create_update({ '_id': new_vlan_name,'vlan': vlan_id, 'name': name, 'type': 'SVLAN', 'vr_name': vr_name, 'vr_status': vr_status, 'lr_name': lr_name, 'lr_status': lr_status, 'ports': ports })
                    else:
                            new_vlan_name = "S-{}".format(vlan_id)
                            _create_update({ '_id': new_vlan_name,'vlan': vlan_id, 'name': name, 'type': 'SVLAN' })
                    ...
                    if len(v._vswitches) > 0:
                        for vs in v._vswitches:
                            ...
                            try:
                                ...
                                _create_update({ '_id': new_vlan_name,'vlan': vlan_id, 'name': name, 'type': 'CVLAN', 'vswitch': vs_name, 'terminating_port': ports, 'terminating_host': pname })
                            except:
        ...
        self.vlaninfo = { 'vlans': sorted(vlans, key=lambda k: int(k['vlan']), reverse=False) }
    ...
    def report_vlan_inventory(self):
        """
            ...
            columns: VLANID, TYPE, VLANNAME, LR_NAME, LR_STATUS, VR_NAME, VR_STATUS, VSWITCH, DEVICES
        """
        fields = [ 'VLAN', 'TYPE', 'NAME', 'LR_NAME', 'LR_STATUS', 'VR_NAME', 'VR_STATUS', 'VSWITCH', 'TERM_HOST', 'TERM_PORT', 'DEVICES' ]
        yield fields
    ...
    def report_switchport_status(self, refresh=False):
        ...
        fields = [ "HOSTNAME", "MGMTIP", "PORT", "ADMINSTATE", "CAPABILITY", "ADMINSPEEDDUPLEX", "OPERSTATE", 
                "OPERSPEEDDUPLEX", "AUTONEG", "LINKSTATEDURATION", "DESCRIPTION", "LLDP", "RSTP", 
                "STORMCONTROL", "XVRSTATUS", "LLDPNBR", "MTU", "EGRESS_QUEUE", "EGRESS_SHAPER", "TYPE", "VPLSPORTTYPE", "LAG_PORTS", "VLANS",
                "ERRORS", "ERR_VLAN", "ERR_MTU", "ERR_SPEED", "ERR_DUPLEX"
              ]
        yield fields
        ...
        for host in self.switchportinfo['nodes']:
            ...
            MGMTIP = UNKNOWN if not hostobj else hostobj.model.get_management_ip()
            ...
            for port in host.get('ports'):
                ...
                r['ADMINSPEEDDUPLEX'] = port.get('adminspeedduplex', UNKNOWN) or NONE
                r['OPERSTATE'] = port.get('operstate', UNKNOWN) or NONE
                r['OPERSPEEDDUPLEX'] = port.get('operspeedduplex', UNKNOWN) or NONE
                r['AUTONEG'] = port.get('autoneg', UNKNOWN) or "off"
                r['LINKSTATEDURATION'] = port.get('linkstateduration', UNKNOWN) or NONE
                ...
                r['VLANS'] = str(self.coldelim.join(port.get('_vlans', []) or []))
                r['TYPE'] = port.get('type', UNKNOWN) or NONE
                r['LAG_PORTS'] = str(self.coldelim.join(port.get('ports', []) or []))
                ...
                r['ERR_SPEED'] = str(port['errors_cat']['speed_issues']['count'])
                r['ERR_DUPLEX'] = str(port['errors_cat']['duplex_issues']['count'])
    ...
    def get_general_inventory(self):
        ...
        for p in self.parserlist:
            ...
            inventory[hostname] = { "mgmtip": mgmtip, "chassisid": chassisid, "vendor": vendor, "hwtype": hwtype, "software": software, "os": os }
    ...
    def report_config_check(self):
        ...
        for p in self.parserlist:
            ...
            ## VERIFY TACACS
            check_server = set(p.model.tacacs.servers or []) == set(['10.0.32.14', '10.0.96.14'])
            check_accounting = p.model.tacacs.accounting == 'enable' and p.model.tacacs.accounting_session == 'on' and p.model.tacacs.accounting_command == 'on'
            check_authorization = p.model.tacacs.authorization == 'enable'
            tacacs = str(check_server and check_accounting and check_authorization)
