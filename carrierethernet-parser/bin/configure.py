...
CARRIERETHERNET_PARSER_LOG_FOLDER = os.environ.get(
    "CARRIERETHERNET_PARSER_LOG_FOLDER", "log"
)
CARRIERETHERNET_PARSER_LOGFILE = os.environ.get(
    "CARRIERETHERNET_PARSER_LOGFILE", "carrier-ethernet-parser"
)
CARRIERETHERNET_PARSER_DIR = os.environ.get(
...
print(CARRIERETHERNET_PARSER_DIR)
...
class Config:
    ...
    CARRIERETHERNET_PARSER_DEBUG_FOLDER = os.environ.get(
        "CARRIERETHERNET_PARSER_DEBUG_FOLDER", os.path.join(SCRIPTDIR, "debug")
    )
    CARRIERETHERNET_PARSER_OUTPUT_FOLDER = os.environ.get(
        "CARRIERETHERNET_PARSER_OUTPUT_FOLDER",
        os.path.join(SCRIPTDIR, "output"),
    )
    CARRIERETHERNET_PARSER_MAX_CONFIG_AGE = int(
        os.environ.get("CARRIERETHERNET_PARSER_MAX_CONFIG_AGE", 4)
    )
    CARRIERETHERNET_PARSER_MAX_PARSED_FILES = int(
        os.environ.get("CARRIERETHERNET_PARSER_MAX_PARSED_FILES", 1200)
    )
